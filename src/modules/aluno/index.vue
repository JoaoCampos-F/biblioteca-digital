  ...
  <v-container class="d-flex justify-center align-center py-10">
    <v-card max-width="90%" class="pa-6 rounded-xl elevation-4" width="100%">
          ...
          <tr v-for="aluno in alunosFiltradosOrdenados" :key="aluno.id">
            ...
            <td>
              <v-row justify="end" no-gutters>
                <v-btn size="small" color="info" class="me-2" @click="editarAluno(aluno)">
                  Editar
                </v-btn>
                <v-btn size="small" color="error" @click="excluirAluno(aluno.id)">
                  Excluir
                </v-btn>
              </v-row>
            </td>
      ...
      <v-dialog v-model="dialog" max-width="600">
        <v-card>
          <v-card-title>
            <span class="text-h6">{{ alunoSelecionado ? 'Editar Aluno' : 'Novo Aluno' }}</span>
          </v-card-title>

          <v-card-text>
            <v-form @submit.prevent="salvarAluno" ref="formRef" v-model="formValido">
              <v-text-field 
                v-model="form.nome" 
                label="Nome" 
...
const rules = {
  ...
  validarNomeUnico: (v: string) => {
    if (!v) return true
    const nomeExistente = alunos.value.find(aluno => 
  ...
  validarMatriculaUnica: (v: number) => {
    if (!v) return true
    const matriculaExistente = alunos.value.find(aluno => 
      aluno.matricula === v && 
      (!alunoSelecionado.value || aluno.id !== alunoSelecionado.value.id)
    )
...
async function validarEmailUnico() {
  ...
  const emailExistente = alunos.value.find(aluno => 
...
async function validarCpfUnico() {
  ...
  const cpfExistente = alunos.value.find(aluno => 
...
async function validarTelefoneUnico() {
  ...
  const telefoneExistente = alunos.value.find(aluno => 
