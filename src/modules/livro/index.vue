  ...
  <v-container class="d-flex justify-center align-center py-10">
    <v-card max-width="90%" class="pa-6 rounded-xl elevation-4" width="100%">
          ...
          <tr v-for="livro in livrosFiltradosOrdenados" :key="livro.id">
            ...
            <td>
              <v-row justify="end" no-gutters>
                <v-btn size="small" color="info" class="me-2" @click="editarLivro(livro)">
                  Editar
                </v-btn>
                <v-btn size="small" color="error" @click="excluirLivro(livro.id)">
                  Excluir
                </v-btn>
              </v-row>
            </td>
      ...
      <v-dialog v-model="dialog" max-width="600">
        <v-card>
          <v-card-title>
            <span class="text-h6">{{ livroSelecionado ? 'Editar Livro' : 'Novo Livro' }}</span>
          </v-card-title>

          <v-card-text>
            <v-form @submit.prevent="salvarLivro" ref="formRef" v-model="formValido">
              <v-text-field 
                v-model="form.titulo" 
                label="Título" 
...
const rules = {
  ...
  quantidade: (v: number) => (v && v > 0) || 'Quantidade deve ser maior que zero',
  dataPublic: (v: string) => {
    if (!v) return true
  ...
  validarTituloUnico: (v: string) => {
    if (!v) return true
    const tituloExistente = livros.value.find(livro => 
