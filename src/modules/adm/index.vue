<template>
  <DefaultLayout>
    <CardContent title="Gerenciar Administradores">
      ...
      <v-dialog v-model="dialog" max-width="600">
        <v-card>
          <v-card-title>
            <span class="text-h6">{{
              adminSelecionado ? "Editar Administrador" : "Novo Administrador"
            }}</span>
          </v-card-title>

          <v-card-text>
            <v-form
              @submit.prevent="salvarAdmin"
              ref="formRef"
              v-model="formValido"
            >
              <v-text-field
...
const rules = {
  ...
  validarNomeUnico: (v: string) => {
    if (!v) return true;
    const nomeExistente = administradores.value.find(
      (admin) =>
  ...
  validarUsuarioUnico: (v: string) => {
    if (!v) return true;
    const usuarioExistente = administradores.value.find(
      (admin) =>
...
async function validarEmailUnico() {
  ...
  const emailExistente = administradores.value.find(
    (admin) =>
...
async function validarCpfUnico() {
  ...
  const cpfExistente = administradores.value.find(
    (admin) =>
...
async function carregarAdministradores() {
  try {
    ...
    const res = await fetch(
      "http://localhost:8080/api/administrador/buscar?all=true"
    );
