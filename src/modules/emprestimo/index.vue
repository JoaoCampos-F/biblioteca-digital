<template>
  <v-container class="d-flex justify-center align-center py-10">
    <v-card max-width="95%" class="pa-6 rounded-xl elevation-4" width="100%">
      ...
      <v-row class="mb-6">
        ...
        <v-col cols="12" md="3">
          <v-card color="success" variant="tonal" class="text-center pa-4">
            <v-card-text>
              <div class="text-h4 font-weight-bold">{{ emprestimosAtivos }}</div>
        ...
        <v-col cols="12" md="3">
          <v-card color="warning" variant="tonal" class="text-center pa-4">
            <v-card-text>
              <div class="text-h4 font-weight-bold">{{ emprestimosVencendo }}</div>
        ...
        <v-col cols="12" md="3">
          <v-card color="error" variant="tonal" class="text-center pa-4">
            <v-card-text>
              <div class="text-h4 font-weight-bold">{{ emprestimosAtrasados.length }}</div>
      ...
      <v-row align="center" class="mb-6">
        ...
        <v-col cols="12" md="3" class="text-md-end text-center">
          <v-btn color="success" @click="abrirNovoEmprestimo">Novo Empréstimo</v-btn>
        </v-col>
      </v-row>
          ...
          <tr v-for="emprestimo in emprestimosFiltrados" :key="emprestimo.id">
            ...
            <td>
              <v-row justify="end" no-gutters>
                ...
                <v-btn size="small" color="info" class="me-2" @click="editarEmprestimo(emprestimo)">
                  Editar
                </v-btn>
                <v-btn size="small" color="error" @click="excluirEmprestimo(emprestimo.id)">
                  Excluir
                </v-btn>
              </v-row>
            </td>
      ...
      <v-dialog v-model="dialog" max-width="600">
        <v-card>
          <v-card-title>
            <span class="text-h6">{{ emprestimoSelecionado ? 'Editar Empréstimo' : 'Novo Empréstimo' }}</span>
          </v-card-title>

          <v-card-text>
            <v-form @submit.prevent="salvarEmprestimo" ref="formRef" v-model="formValido">
              <v-select
        ...
        <v-card>
          ...
          <v-card-text>
            <v-table v-if="emprestimosAtrasados.length > 0">
              ...
              <tbody>
                <tr v-for="emprestimo in emprestimosAtrasados" :key="emprestimo.id">
                  <td>{{ getNomeAluno(emprestimo.id_aluno) }}</td>
                  <td>{{ getTituloLivro(emprestimo.id_livro) }}</td>
                  <td>{{ formatarData(emprestimo.data_vencimento) }}</td>
                  <td>
...
const emprestimosAtivos = computed(() => emprestimos.value.filter(e => e.is_active && !e.data_devolucao).length)
...
const livrosDisponiveis = computed(() => livros.value.filter(l => l.quantidade >= 1))
...
const emprestimosFiltrados = computed(() => {
  ...
  if (filtro.value) {
    resultado = resultado.filter(e => 
      getNomeAluno(e.id_aluno).toLowerCase().includes(filtro.value.toLowerCase()) ||
      getTituloLivro(e.id_livro).toLowerCase().includes(filtro.value.toLowerCase())
    )
  }
  ...
  if (filtroStatus.value) {
    resultado = resultado.filter(e => {
      switch (filtroStatus.value) {
        ...
        case 'vencido':
          return e.is_active && !e.data_devolucao && new Date(e.data_vencimento) < new Date()
        default:
          return true
      }
    })
...
async function carregarEmprestimos() {
  try {
    ...
    const res = await fetch('http://localhost:8080/api/emprestimo/buscar?all=true')
    ...
    emprestimos.value = Array.isArray(data) ? data : data.content || data.data || []
  } catch (err) {
...
async function carregarAlunos() {
  try {
    ...
    if (res.ok) {
      const data = await res.json()
      alunos.value = Array.isArray(data) ? data : data.content || data.data || []
    }
  } catch (err) {
...
async function carregarLivros() {
  try {
    ...
    if (res.ok) {
      const data = await res.json()
      livros.value = Array.isArray(data) ? data : data.content || data.data || []
    }
  } catch (err) {
...
async function mostrarAtrasos() {
  try {
    ...
    if (res.ok) {
      const data = await res.json()
      emprestimosAtrasados.value = Array.isArray(data) ? data : data.content || data.data || []
...
async function excluirEmprestimo(id: string) {
  ...
  try {
    const res = await fetch(`http://localhost:8080/api/emprestimo/excluir/${id}`, {
      method: 'DELETE',
    })
